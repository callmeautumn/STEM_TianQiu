...
class Animator:
    ...
    def scale(self, c, s =1):
        return self.transform(c, [[s,0],[0,s]])
    ...
    def linear_transformation(self, src, a):
        ...
        indices = x + N*y
        new_image = [np.take(src[:,:,c], indices, mode='raise') for c in range(C)]
        new_image = np.stack(new_image, axis=2)
        return new_image
    
    def transform(self, canvas, m):
        return self.linear_transformation(canvas, m)
